...
from lib.models.speech_cnn_ctc import *
...
def run_training(H):
    ...
    model = SpeechCNN(len(vocab), input_size=256, hidden_size=H.CNN_HIDDEN_SIZE,
                      dropout=H.CNN_DROPOUT, initialize=torch_weight_init)
    if H.USE_CUDA:
        model.cuda()
...
def run_evaluation(H):
    ...
    model_pred = SpeechCNN(len(vocab), input_size=256, hidden_size=H.CNN_HIDDEN_SIZE,
                           dropout=H.CNN_DROPOUT, initialize=torch_weight_init)
    if H.USE_CUDA:
        model_pred.cuda()
